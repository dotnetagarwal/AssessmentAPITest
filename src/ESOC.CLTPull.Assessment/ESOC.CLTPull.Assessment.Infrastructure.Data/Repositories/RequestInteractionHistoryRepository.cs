...
{
    /// <inheritdoc />
    public sealed class RequestInteractionHistoryRepository : Repository<RequestInteractionHistory>, IRequestInteractionHistoryRepository
    {
        protected readonly CLTPULLContext _dbContext;
        private readonly IConfiguration _configuration;
        public RequestInteractionHistoryRepository(CLTPULLContext dbContext, IConfiguration configuration) : base(dbContext)
        {
            _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
            _configuration = configuration;
        }
        public List<RequestInteractionHistoryListDataDTO> GetRequestInteractionHistoryListData()
        {
            string ConnectionString = _configuration.GetConnectionString(SqlConstants.cltpullDb);
            string query = SqlConstants.vw_GetAllRequestInteractionHistoryData;
            AdoHelper.Connect(ConnectionString);
            SqlDataReader reader = AdoHelper.ExecDataReader(query);
            List<RequestInteractionHistoryListDataDTO> requestInteractionHistoryListDataDTO = new List<RequestInteractionHistoryListDataDTO>();
            RequestInteractionHistoryListDataDTO requestInteractionHistoryDataDTO = null;
            while (reader.Read())
            {
                requestInteractionHistoryDataDTO = new RequestInteractionHistoryListDataDTO
                {
                    RequestInteractionHistoryId = reader.Value<Int32>("RequestInteractionHistoryId"),
                ...
                requestInteractionHistoryListDataDTO.Add(requestInteractionHistoryDataDTO);
            }
            reader.Close();
            AdoHelper.Dispose(true);
            return requestInteractionHistoryListDataDTO;
        }
    }
}
